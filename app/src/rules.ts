...
export function unavailableReason(
  ...
  if (selected.reduce((s, m) => s + m.cost, 0) + measure.cost > config.budget)
    return 'Недостаточно бюджета'
  if (selected.filter((m) => m.category === measure.category).length >= config.max_per_category)
    return 'В этом направлении уже две меры'
  for (const conflict of config.incompatibilities) {
    ...
    if (other && (conflict.scope === 'city' || other.district_id === district))
