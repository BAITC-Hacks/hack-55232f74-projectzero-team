...
export type CityResult = {
  ...
  average: number
  minimum: number
  weakest_district_id: string
  critical: { district_id: string; metric_id: string; value: number }[]
  districts: DistrictResult[]
  synergies: { pair: string[]; effects: Record<string, number>; district_id: string }[]
}
...
export type Result = CityResult & {
  baseline_score: number
  score_delta: number
  spent: number
  remaining: number
  decisions: Decision[]
  contributions: (Decision & { score_delta: number })[]
...
export type Explanation = {
  ...
  report: {
    summary: string
    strengths: { text: string; fact_ids: string[] }[]
