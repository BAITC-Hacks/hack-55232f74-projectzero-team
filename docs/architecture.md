...
Сеть и здания процедурные, независимы от тайлов OSM. При отсутствии WebGL используется SVG-представление того же состояния. Для повторяющихся объектов применяется instancing; статические тени рассчитываются один раз, на программном GPU отключаются. При паузе и неподвижной камере сцена не перерисовывается. Транспортная статистика не подменяет серверный Score и не передаётся LLM как прогноз реального города. Параметры описаны в [модели транспорта](traffic.md).
