...
```mermaid
flowchart LR
    UI[React + Leaflet + Three.js] -->|решения| API[FastAPI]
    UI -->|меры и датасет| Traffic[Движок трафика в браузере]
    Traffic -->|машины, очереди, пассажиры| UI
    API --> Engine[Валидатор и расчёт]
    Data[scenario.json] --> Engine
    Geo[Снимок OSM: astana.json] --> Traffic
    Geo --> UI
...
## 3D-город и дорожная симуляция

`GameView` загружается отдельным JS-модулем; выбор мер хранится в `App` и доступен также аналитическому экрану. `TrafficEngine` не зависит от React, DOM или Three.js: он обновляет машины и пассажирские очереди с фиксированным шагом. `CityRenderer` отображает полученное состояние и обслуживает камеру. Базовый и текущий движки развиваются на одном модельном времени для сравнения.

Сеть основана на включённом в сборку снимке OpenStreetMap, обычные здания процедурные. Во время работы 3D-сцена не обращается к Overpass, Nominatim или сервисам тайлов. `scripts/import-astana.mjs` преобразует предварительно сохранённый ответ Overpass в компактный граф; геометрия дорог и источник данных описаны в [модели Астаны](astana.md). Достопримечательности создаются в `landmarks.ts`, окружение — в `cityScene.ts`.

При отсутствии WebGL используется SVG-представление того же состояния, с географией и ориентирами. Для повторяющихся объектов применяется instancing; дороги объединены в общие меши; статические тени рассчитываются один раз, на программном GPU отключаются. При паузе и неподвижной камере сцена не перерисовывается. Транспортная статистика не подменяет серверный Score и не передаётся LLM как прогноз реального города. Параметры описаны в [модели транспорта](traffic.md).
